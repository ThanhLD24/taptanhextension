...
function timer()
{
    ...
    var countDownPercent = countDown / 30 * 100;
    if(countDown >= 16) {
        $('#updatingInBar').removeClass('progress-bar-danger');
        $('#updatingInBar').removeClass('progress-bar-warning');
        $('#updatingInBar').addClass('progress-bar-success');
    } else if(countDown <= 15 && countDown >= 6) {
        $('#updatingInBar').removeClass('progress-bar-danger');
        $('#updatingInBar').removeClass('progress-bar-success');
        $('#updatingInBar').addClass('progress-bar-warning');
    } else {
        $('#updatingInBar').removeClass('progress-bar-warning');
        $('#updatingInBar').removeClass('progress-bar-success');
        $('#updatingInBar').addClass('progress-bar-danger');
    }
    $('#updatingInBar').css('width', countDownPercent+"%");
}
